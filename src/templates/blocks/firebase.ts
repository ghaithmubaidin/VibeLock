  ...
  return {
    ...
    source:
      fp.detectedFiles?.filter(
        (f) => f.includes('firebase') || f === 'package.json',
      ) ?? [],
    globs: ['firebase.json', 'firestore.rules', 'database.rules', 'storage.rules', '**/*firebase*', 'src/lib/firebase/**/*', 'lib/firebase/**/*'],
    description: 'Firebase SDK initialization, Security Rules, App Check, and Cloud Functions guidelines',
